...
max_size = max([len(d) for d in grid_data])  # Максимальный размер S
S = np.zeros((max_size,))  # Создание массива S с максимальным размером
# -----------------------------------------------
# init
dl = 3
subblock_size = [6,5,5]
                ...
                for i in range(0, len(origin2), coef):
                    ...
                    for j in range(coef):
                        ...
                        if len(lp[j]) > 0 and len(rp[j]) > 0:
                            ...
                            _, s, Vj = np.linalg.svd(combined_points)
                            S[:len(s)] = s
                            sv1, sv2 = S[0], S[1]
                            main_direction_j = Vj[0]
                            energy = (sv1 - sv2) / (sv1 + sv2)
                            first_principal_orientations += main_direction_j

                        elif len(lp[j]) > 0 and len(rp[j]) == 0:
                            center_l = np.mean(lp[j], axis=0)
                            centered_l = center_l - center_o
                            _, s, Vl = np.linalg.svd(centered_l.reshape(1, -1))
                            S[:len(s)] = s
                            sv1, sv2 = S[0], S[1]
                            ...
                            center_r = np.mean(rp[j], axis=0)
                            centered_r = center_r - center_o
                            _, s, Vr = np.linalg.svd(centered_r.reshape(1, -1))
                            S[:len(s)] = s
                            sv1, sv2 = S[0], S[1]
                        ...
                        X = np.sum(energy * lp[j][:,0])
                        Y = np.sum(energy * lp[j][:,1])
                        Z = np.sum(energy * lp[j][:,2])

                        sum_ER = np.sum(energy)

                        X /= sum_ER
                        Y /= sum_ER
                        Z /= sum_ER

                        main_vector = np.array([X, Y, Z])
